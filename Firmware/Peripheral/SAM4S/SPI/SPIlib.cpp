...
void InitSPI()
{
	...
	/* SPI to ADS1118 */
	// Set SPI-Mode 1
	SPI->SPI_CSR[1] &= ~(SPI_CSR_CPOL);
	SPI->SPI_CSR[1] &= ~(SPI_CSR_NCPHA);
	// Set 8 bits per transfer
	SPI->SPI_CSR[1] |= SPI_CSR_BITS_8_BIT;
	// Keep chip selected after transfer (manual finish)
	SPI->SPI_CSR[1] |= SPI_CSR_CSAAT;
	// Set bautrate to 6 Mhz
	SPI->SPI_CSR[1] &= (~SPI_CSR_SCBR_Msk);
	SPI->SPI_CSR[1] |= SPI_CSR_SCBR(80);
	// Set Chip-Select setup and multiple transfer delay
	SPI->SPI_CSR[1] &= ~(SPI_CSR_DLYBS_Msk | SPI_CSR_DLYBCT_Msk);
	SPI->SPI_CSR[1] |= SPI_CSR_DLYBS(0x0) | SPI_CSR_DLYBCT(0);
